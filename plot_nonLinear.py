...
fontdict = {'fontsize': 13, 'fontweight': 'normal', 'fontfamily':'monospace'}

matrix1 = pd.pivot_table(df1, values="torque", index=['theta'], columns=['js'], aggfunc=np.mean)
matrix2 = pd.pivot_table(df2, values="torque", index=['theta'], columns=['js'], aggfunc=np.mean)
...
for i in range(len(js)):
    ...
    ax.text(0.85, 0.83, "k = {:2.0f}".format(js[i]/js0), fontsize=fontsize,
            bbox=dict(facecolor='forestgreen', edgecolor='none', alpha=0.5),
...
fig.text(0.01, 0.5, 'Couple [N m]', va='center', rotation='vertical', fontsize=fontsize)

(fig.get_axes()[0]).set_title("Relation Angle - Couple", fontsize=fontsize+1)
