...
if __name__ == "__main__":
    ...
    fig, ax = plt.subplots(1, 1, figsize=(
        10, 4.5), constrained_layout=True, sharex='all', sharey='all')
    ...
    matrix = pd.pivot_table(df, values="torque", index=[
                            'theta'], columns=['coil'], aggfunc=np.mean)
    theta = matrix.index
    ...
    for i in range(len(coils)):
        ax.plot(theta, matrix[:, i], color='C' +
                str(i), label=labels[i], alpha=0.5)
