...
if __name__ == "__main__":
    ...
    dfs = [pd.read_csv(filename, delimiter=" ", names=names,
                       index_col=False) for filename in files]

    for df in dfs:
        df.drop(df[df.theta < 1e-5].index, inplace=True)
        df.loc[df.theta > 2 * np.pi - 1e-3, 'theta'] = 0.

    matrices = [1.e3 * pd.pivot_table(df, values="time", index=['theta'], columns=[
                                      'nNode'], aggfunc=np.mean) for df in dfs]
    nNode = matrices[0].columns

    ############################################## - FIGURE TIME - ##############################################

    fig, ax = plt.subplots(1, 1, figsize=(
        6, 8), constrained_layout=True, sharex='all', sharey='all')

    for i, size in enumerate(nNode):
        line = ax.plot(np.degrees(matrices[0].index), matrices[0][size],
                       '-o', color='C'+str(i), ls='-', label="{:>6d} noeuds".format(size))
        # , label="{:d} once".format(size))
        line = ax.plot(np.degrees(
            matrices[1].index), matrices[1][size], '-o', color='C'+str(i), ls='--')
        # , label="{:d} none".format(size))
        line = ax.plot(np.degrees(
            matrices[2].index), matrices[2][size], '-o', color='C'+str(i), ls=':')
        ax.set_yscale("log")

    handles, labels = ax.get_legend_handles_labels()
    first_legend = ax.legend(handles[::-1], labels[::-1], loc='center right', prop={
                             'family': 'monospace', 'size': fontsize2})
    ...
    #     else:
    #         pos = 0.85
    #     text = ax.text(pos, .89, title, size=12,
