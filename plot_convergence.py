...
model = LinearRegression().fit(np.log(L[:-1]).reshape((-1,1)), np.log(L2_C))
r_sq = model.score(np.log(L[:-1]).reshape((-1,1)), np.log(L2_C))
intercept = model.intercept_
orderM, = model.coef_
print("L2 order in n = {:.3f} \t R2 = {:.1f} \t intercept = {:.2f}".format(orderM, 100*r_sq, intercept))

model = LinearRegression().fit(np.log(L[:-1]).reshape((-1,1)), np.log(H1_C))
r_sq = model.score(np.log(L[:-1]).reshape((-1,1)), np.log(H1_C))
intercept = model.intercept_
orderM, = model.coef_
print("H1 order in n = {:.3f} \t R2 = {:.1f} \t intercept = {:.2f}".format(orderM, 100*r_sq, intercept))


################################################ - FIGURE - #################################################

fig, axs = plt.subplots(2, 1, figsize=(10,5.5), constrained_layout='all')
ax1 = axs[0]
ax2 = axs[1]

######### FIRST PLOT
ax1.loglog(L, L2_A, '-o', color='C0', label='$||\; a_i \;||$');
ax1.loglog(L[:-1], L2_B, '-o', color='C1', label='$||\; a_{i} - a_{i+1} \;||$');
ax1.loglog(L[:-1], L2_C, '-o', color='C2', label='$||\; a_{i} - a_{ref} \;||$');
...
ax2.loglog(L[:-1], H1_B, '-o', color='C1', label='$||\; a_{i} - a_{i+1} \;||$');
ax2.loglog(L[:-1], H1_C, '-o', color='C2', label='$||\; a_{i} - a_{ref} \;||$');
