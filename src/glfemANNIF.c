...
void displayCredits(animation *myAnim) {
	...
	float wSerie[3] = {w*.5f, w*.6f, w*.5f};
	float hSerie[3] = {h*.7f, h*.45f, h*.2f};
	float bSerie[3] = {wSerie[0]*3./9., wSerie[1]*3./11., wSerie[2]*3./8.};
	char sSerie[][13] = {"Joyeux", "Anniversaire", "Maxime !"};
    ...
    bov_text_set_pos(text1, (GLfloat[2]) {(w - wSerie[0])*0.5f, hSerie[0]});
    bov_text_set_pos(text2, (GLfloat[2]) {(w - wSerie[1])*0.02f, hSerie[1]});
    bov_text_set_pos(text3, (GLfloat[2]) {(w - wSerie[2])*0.35f, hSerie[2]});
    ...
    switch (action) {
        ...
        case 'C':
            if (myAnim->mode == 3) {
                ...
                myAnim->flag = 0;
                myAnim->delayTime = glfwGetTime();
                myAnim->funStartTime = glfwGetTime()+3.;
...
void updateDisplay(animation *myAnim) {
    ...
    else if (myAnim->mode == 3 && myAnim->delayTime + 3. < glfwGetTime()) {
