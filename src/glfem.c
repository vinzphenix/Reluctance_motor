...
void easterEgg(animation *myAnim, int n) {
    ...
    GLfloat coordMOVE[][2] = {{650.f * ratioW, 10 * ratioH}, {end * ratioW, 10 * ratioH}, {end * ratioW, 30. * ratioH}, {end * ratioW, 29.8 * ratioH}, {650.f * ratioW, 29.8 * ratioH}, {650.f * ratioW, 9.8 * ratioH}};
...
void displayMode0(animation *myAnim, motor *theMotor, int subMode, femMesh **theDomains) {
    ...
    femMesh *theStator = theDomains[0];
    femMesh *theGap = theDomains[11];
    femMesh *theCoil_AP = theDomains[1];
    femMesh *theCoil_AN = theDomains[2];
    femMesh *theCoil_BP = theDomains[3];
    femMesh *theCoil_BN = theDomains[4];
    femMesh *theCoil_CP = theDomains[5];
    femMesh *theCoil_CN = theDomains[6];
    femMesh *theStatorGap = theDomains[7];
    femMesh *theRotor = theDomains[8];
    femMesh *theRotorAir = theDomains[9];
    femMesh *theRotorGap = theDomains[10];
...
void displayButton(animation *myAnim) {
    ...
    if (myAnim->running)
        playButton(myAnim, coordPLAY);
    else
        pauseButton(myAnim, coordPAUSE);
}
...
void updateParameters(animation *myAnim) {
    ...
    if (myAnim->stopTime <= myAnim->time)
        myAnim->time = myAnim->stopTime;
    ...
    case 'P':
        if (myAnim->running)
            makePause(myAnim);
        else if (myAnim->mode == 0 || myAnim->mode == 1)
            makePlay(myAnim);
        break;
    ...
    case 'R':
        ...
        for (int i = 0; i < theMotor->size; i++) {
            ...
            if (theMotor->movingNodes[i])
                theMotor->a[i] = sin(3 * th);
            else
                theMotor->a[i] = -sin(3 * th);
        }
