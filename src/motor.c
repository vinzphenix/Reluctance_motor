...
int compareijk (const void *_a, const void *_b) {
	if (((ijk*)_a)->i < ((ijk*)_b)->i) return -1;
	else if (((ijk*)_a)->i > ((ijk*)_b)->i) return 1;
	else if (((ijk*)_a)->j < ((ijk*)_b)->j) return -1;
	else if (((ijk*)_a)->j > ((ijk*)_b)->j) return 1;
	return 0;
}
...
void getMu(const double *B, const double *H, int n, double *ddH, double b2, double db2, double *mu) {
    ...
    for (int i = 0 ; i < n-1 ; i++) {
        if (B[i] <= b_mid && b_mid < B[i+1]) {
            ...
            w1 = B[i+1] - b_mid;
            w2 = b_mid - B[i];
            mu[0] =  ddH[i]   / (6. * Db) * w1 * w1 * w1
                   + ddH[i+1] / (6. * Db) * w2 * w2 * w2
                   + (H[i] / Db   - ddH[i]   * Db / 6.) * w1
                   + (H[i+1] / Db - ddH[i+1] * Db / 6.) * w2; // here we get H(b)
        ...
        if (B[i] <= b_prev && b_prev < B[i+1]) {
            ...
            w1 = B[i+1] - b_prev;
            w2 = b_prev - B[i];
            mu[1] =  ddH[i]   / (6. * Db) * w1 * w1 * w1
                   + ddH[i+1] / (6. * Db) * w2 * w2 * w2
                   + (H[i] / Db   - ddH[i]   * Db / 6.) * w1
                   + (H[i+1] / Db - ddH[i+1] * Db / 6.) * w2;
            mu[1] /= b_prev;}
...
void assembleJacobian(motor *theMotor, sparseSolver *theSolver) {
    ...
    for (iElem = 0 ; iElem < theMesh->nElem ; iElem++) {
        ...
        for (i = 0 ; i < nLoc*nLoc ; i++) Aloc[i] = 0.;
        for (i = 0 ; i < nLoc ; i++) Bloc[i] = 0.;
        memset(Aloc, 0, nLoc * nLoc * sizeof(double));
        memset(Bloc, 0,        nLoc * sizeof(double));
        getLocalInfo(theMotor, number, dirichlet, iElem, map, ctr, Xloc, Yloc, Uloc);
...
void setupReMesh(motor *theMotor, motorReMesh *theReMesh) {
    ...
    int currentNode = -1;
    for (i=0, n=0 ; i < N ; i++) {
        if (currentNode == rotorNodes[i]) continue;
    ...
    currentNode = -1;
    for (i=0, m=0 ; i < M ; i++) {
        if (currentNode == statorNodes[i]) continue;
...
double elementQuality(double *x, double *y) {
    ...
    double per = hypot(x[1] - x[0], y[1] - y[0]) 
...
void checkAndSwap(motor *theMotor, motorReMesh *theReMesh, double delta) {
    ...
    X[0] =  theMesh->X[rotorNodes[0]] ; Y[0] =  theMesh->Y[rotorNodes[0]];
    X[1] = theMesh->X[statorNodes[0]] ; Y[1] = theMesh->Y[statorNodes[0]];
...
void motorMeshRenumber(motorMesh *theMesh, int *number) {
    ...
    for (int i = 0 ; i < theMesh->nNode ; i++) inverse[i] = i; 
   
    if (1) theGlobalCoord = theMesh->X;
    if (0) theGlobalCoord = theMesh->Y;
    
    qsort(inverse, theMesh->nNode, sizeof(int), compareByCoord);
    for (int i = 0 ; i < theMesh->nNode ; i++) number[inverse[i]] = i;
...
void motorFree(motor *theMotor) {
    ...
    if (globalReMesh) freeReMesh(globalReMesh);
    if (number) free(number);
    if (dirichlet) free(dirichlet);
    if (ddH) free(ddH);
