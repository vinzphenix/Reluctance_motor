...
df.insert(len(df.columns)-1, 'other', df['full'] - df[s.drop("full")].sum(axis=1),True)
s = df.columns.drop(["nNode", "nIter"])
...
######### SECOND PLOT
ax = axs[1]
colors = [cmap(0. + i/7 * 1.) for i in range(6)] + ['C3', 'C1', 'C0', 'lightgrey']
width = 0.25
...
for i in range(len(s) - 1):
    ...
    ax.bar(ind, 1e2 * y1, width, bottom=bottom, label=s[i], color=colors[i])
...
ax.legend(handles[::-1], labels[::-1], prop={'family': 'monospace', 'size': fontsize2}, ncol=2)
